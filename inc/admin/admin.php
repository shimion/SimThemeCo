...
require _ADMIN. "wp-editor-widget/wp-editor-widget.php";
require _ADMIN. "export-import/import.php";
require _ADMIN. "export-import/export.php";


// Remove Visual Content Editor For Builder Page tempalte (template.php)
...
function hide_editor() {
    ...
    if($template_file == 'template.php'){ // edit the template name
...
function AdminAjaxData(){
    ...
    //note: When add a new page it will automatically create a dtraft page and so probably it works with update_posts always
    // Do not need to do maybe_serialize during updating the post meta. Somehow it automatically fit here. Need to go through more depth on it.
   ...
   if(empty($post_ID)){
       echo $post_ID;
        ...
        //add_post_meta( $postID, '_custom_page_options[customizer]', 'true' );
       ...
       if($postID){
           $customizer_data = [];
           $meta_data = [];
           $meta_data = get_post_meta( $postID, '_custom_page_options', false );
           $meta_data = maybe_unserialize($meta_data);
            $exxmeta_data = $meta_data[0];
           if($exxmeta_data['customizer'])
           $exxmeta_data['customizer'] = false;
           else{
            $exxmeta_data['customizer'] = true;
           }


            if ( ! add_post_meta( $postID, '_custom_page_options', $exxmeta_data, true ) ) {
                    update_post_meta( $postID, '_custom_page_options', $exxmeta_data );
                }


        if($exxmeta_data['customizer'] == 1){
        $page_template_meta = update_post_meta( $postID, '_wp_page_template', 'template.php' );
         ...
         $page_template_meta = update_post_meta( $postID, '_wp_page_template', 'page.php' );
...
$wp_customize->get_setting( '_cs_customize_options[site_title]' )->transport = 'postMessage';

    $wp_customize->selective_refresh->add_partial( '_cs_customize_options[site_title]', array(
    'selector' => 'header .site-title-description-wapper',
    'render_callback' => get_option('_cs_customize_options[site_title]'),
) );



$wp_customize->get_setting( '_cs_customize_options[site_tagline]' )->transport = 'postMessage';

    $wp_customize->selective_refresh->add_partial( '_cs_customize_options[site_tagline]', array(
    'selector' => 'header .site-title-description-wapper span',
    'render_callback' => get_option('_cs_customize_options[site_tagline]'),
) );


$wp_customize->get_setting( '_cs_customize_options[footer_backgrounds]' )->transport = 'postMessage';

    $wp_customize->selective_refresh->add_partial( '_cs_customize_options[footer_backgrounds]', array(
    'selector' => 'footer#footer',
    'render_callback' => get_option('_cs_customize_options[footer_backgrounds]'),
) );


$wp_customize->get_setting( '_cs_customize_options[footer_backgrounds_widget]' )->transport = 'postMessage';

    $wp_customize->selective_refresh->add_partial( '_cs_customize_options[footer_backgrounds_widget]', array(
    'selector' => 'section.footer-widget-section',
    'render_callback' => get_option('_cs_customize_options[footer_backgrounds_widget]'),
) );
...
function enqueue_datepicker_uistyles( $hook_suffix ) {
	...
	$screen = get_current_screen();
        ...
        wp_enqueue_style( 'jquery.ui.theme', ADMIN . '/option/assets/css/ui-date-picker.css' , array( 'jquery-ui-core', 'jquery-ui-datepicker' ), VERSION, 'all' );
...
add_action('admin_footer', function(){
    ...
    ?>
    <style>
        .ui-widget.ui-widget-content[aria-describedby="cs-shortcode-dialog"]{z-index: 99999  !important;}
...
add_action('wp_footer', 'Customizer_Preview_specific_CSS_for_editor_icon');
function Customizer_Preview_specific_CSS_for_editor_icon(){
    if(is_customize_preview()):
    ?>
<style>
span.customize-partial-edit-shortcut.customize-partial-edit-shortcut-_cs_customize_options-footer_backgrounds, span.customize-partial-edit-shortcut.customize-partial-edit-shortcut-_cs_customize_options-footer_backgrounds_widget {
    top: -40px;
    left: 32px;
}

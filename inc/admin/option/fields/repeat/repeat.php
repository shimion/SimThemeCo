<?php 
class CSFramework_Option_repeat extends CSFramework_Options {
  ...
  public function output() {
    ...
    echo '<div class="cs-group cs-group-'. $el_class .'- adding hidden">';
      ...
      foreach ( $fields as $field ) {
        ...
        $unique         = $this->unique .'[_nonce]'. $this->field['name'] .'[_ex_nonce]['. $field['id'] .']';
           $field['name'] =   $unique ;
        $field_default  = ( isset( $field['default'] ) ) ? $field['default'] : '';
        echo cs_add_element( $field, $field_default, $unique );
      }
      echo '<div class="cs-element cs-text-right cs-remove"><a href="#" class="button cs-warning-primary cs-remove-group">'. __( 'Delete', 'cs-framework' ) .'</a></div>';
      echo '</div>';

    echo '</div>';
        $this->value = maybe_unserialize($this->value);
        $count = count($this->value);
    echo '<div class="cs-groups sub-cs-accordion ">';
      ...
      if( ! empty( $this->value ) ) {
        ...
        foreach ( $this->value as $key => $value ) {
          ...
          echo '<h4 class="cs-group-title-sub">'. $field_title .' '. $title .'</h4>';
          echo '<div class="cs-group-content">';

          foreach ( $fields as $field ) {
           $field['sub'] = true;
           
            $unique = $this->unique  . $this->field['id'] . '['.$key.']'.'['.$field['id'].']';
               $field['name'] =   $unique ;
            $subvalue  = ( isset( $field['id'] ) && isset( $this->value[$key][$field['id']] ) ) ? $this->value[$key][$field['id']] : '' ;;
            echo cs_add_element( $field, $subvalue, $unique );
                   // print_r($field);
          }

...
Run Via ajax call
example: http://mercer.simtheme.com/wp-admin/admin-ajax.php?action=Import_Layer
*/
add_action( 'wp_ajax_AdminAjaxImport', 'AdminAjaxImport' );
add_action( 'wp_ajax_nopriv_AdminAjaxImport', 'AdminAjaxImport' );
function AdminAjaxImport(){
    $data = $_POST['data'];
    $path = $path['basedir'];
    global $id;
    $id = $_POST['post_ID'];
   // echo $path.'/2017/12/file-1.json';
   // echo $path;
   // $path = $path.'/data/file.json';
  // $path = 'http://heartforuganda.simtheme.com/file.json';
   // echo $path;
    
  $encoded_data = st_process_import_file($data, $id);
   ...
   // echo 'Shimion:' .$id;
    ...
    global $st_import_results;
    //print_r($st_import_results);
     exit();
}


function st_upload_import_file() {
$path = wp_upload_dir();
   // print_r($path);
    $path = $path['basedir'];
   // echo $path.'/2017/12/file-1.json';
 ...
 //   $path = $path.'/data/file.json';
	...
	if ( ! empty( $data_cm ) ) {
       ...
       // print_r($uploaded_file);
		...
		// Process import file.
		st_process_import_file( $file_data['file'] );

	}

}

add_action( 'load-tools_page_widget-importer-exporter', 'st_upload_import_file' );
 ...
 * @global string $st_import_results
 */
function st_process_import_file( $file, $id ) {
	...
	$st_import_results = st_import_data( $data, $id );
  //print_r($st_import_results);
   // return $st_import_results;
}
 ...
 */
function st_import_data( $data, $id) {
   ...
   // print_r($data);
	...
	// Hook before import.
	do_action( 'st_before_import' );
	$data = apply_filters( 'st_import_data', $data );
   ...
   // print_r($available_widgets);
	...
	foreach ( $data as $sidebar_id => $widgets ) {
      ...
      //echo $id.'<br>'.$sidebar_id;
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			// Ideally the newer st_widget_settings_array below will be used instead of this.
			$widget = apply_filters( 'st_widget_settings', $widget );
			...
			// This is preferred over the older st_widget_settings filter above
			// Do before identical check because changes may make it identical to end result (such as URL replacements).
			$widget = apply_filters( 'st_widget_settings_array', $widget );
			...
			if ( ! $fail ) {
				...
				do_action( 'st_after_widget_import', $after_widget_import );
	...
	// Hook after import.
	do_action( 'st_after_import' );

	// Return results.
    print_r($results);
	return apply_filters( 'st_import_results', $results );

}

/**
 * st_available_widgets: Available widgets
 *

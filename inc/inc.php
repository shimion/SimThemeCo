...
add_filter('Action_WidgetModule', function($instance){
    //if( ! is_singular())
    return Helpers::Module($instance);
}, 20);
...
add_action('wp_footer', function(){
    if(current_user_can('administrator')){
        $html = '<input type="hidden" class="data_ID" data-id="'.get_the_ID().'" />';
        echo $html;
    }
}, '99999');

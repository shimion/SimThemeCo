...
class Helpers
{
    ...
    public static function AddAdditionalScript(array $newValue, $type = 'css'){
        global $st_layer_scripts;
        $option = unserialize($st_layer_scripts);

         $data = $option;
      //print_r($option);


        if ( $option[$type] != false or !empty($option[$type]) or $option[$type] != null) {
   ...
   public static function GetCallToAction($data){
            $CallToAction = new CallToAction($data);
           return $CallToAction->output();
    }
   ...
   public static function RestrictSidebar(){
    ...
    public function Image(){
        ...
        if(!empty($this->image['href']))
            $image = sprintf('<a class="navbar-brand" href="%s">%s</a>', $this->image['href'], $image);
...
public static function GenerateDynamicJs(){
    return '';
    }
